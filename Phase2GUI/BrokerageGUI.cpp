...
Patron::Patron()
{
	...
	const double am = 0.0;
	// make share objects for the stocks
	Share intc = Share(Stock("INTC"), am);
	Share goog = Share(Stock("GOOG"), am);
	Share aapl = Share(Stock("AAPL"), am);
	Share yhoo = Share(Stock("YHOO"), am);
	Share ibm = Share(Stock("IBM"), am);
	// add the stocks to the shares
	shares.push_back(intc);
	shares.push_back(goog);
	shares.push_back(aapl);
	shares.push_back(yhoo);
...
{
	if(s.size() < 3) { // name has to be at least 3 characters long
		return false;
	}
	...
	for(int i = 0; i < s.size(); i++) {
		if(isspace(s.at(i))) { // check for spaces
			return false;
		}
		if(s.at(i) == '_') { // check for underscores
			underscores_found++;
		}
	}
	if(underscores_found != 1) { // make sure theres only one underscore
		return false;
	}
...
void Brokerage::add_trans(Transaction n) // adds the transaction to the brokerage
{
